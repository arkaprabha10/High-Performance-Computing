...
if(my_rank==0)
{
     ...
     for(int source=1;source<p;source++)
    {
        ...
        for (j = start; j < end; j++) {
            tableau[global_min.index][j] = temp_2[global_min.index][j];
        }
    }
    // temp_3.clear();
}
...
int error_temp;
if(my_rank!=0)
     error_temp=MPI_Send(&tableau[0][0],nL*nC,MPI_DOUBLE,0,0,MPI_COMM_WORLD);
...
if(my_rank==0)
{
    ...
    for(int source=1;source<p;source++)
    {
        ...
        for (i = start; i < end; i++) 
        {
            tableau[constraintNumb][i] = temp_2[constraintNumb][i];
